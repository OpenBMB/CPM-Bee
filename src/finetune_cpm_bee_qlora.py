...
from cpm_live.layers import Linear4bit
...
def get_model(args):
    ...
    for name, module in model.named_modules():
        if isinstance(module, Linear4bit):
            original_dtype_dict[name] = module.weight.data.dtype
            module.weight.data = module.weight.data.to(torch.half)
    ...
    for name, module in model.named_modules():
        if name in original_dtype_dict:
            module.weight.data = module.weight.data.to(original_dtype_dict[name])
        if isinstance(module, LowRankLinear):
            module.lora_A = nn.Parameter(module.lora_A.data.to(model.config.dtype))
            module.lora_B = nn.Parameter(module.lora_B.data.to(model.config.dtype)) 
    return model
